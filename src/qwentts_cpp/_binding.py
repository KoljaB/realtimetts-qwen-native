...
class LibraryLoadError(QwenTTSError):
    """The native library exists but one of its runtime dependencies could not load."""


class ABIMismatchError(QwenTTSError):
    """The bundled Python structures and native qwentts.cpp ABI do not match."""


class CudaRuntimeError(QwenTTSError):
    """CUDA could not be initialized with the installed GPU driver/runtime."""


class OutOfMemoryError(QwenTTSError):
    """The native backend could not reserve enough CPU or GPU memory."""
...
def _dependency_names() -> Sequence[str]:
    if sys.platform == "win32":
        return ("ggml.dll", "ggml-base.dll", "ggml-cpu.dll", "ggml-cuda.dll")
    if sys.platform == "darwin":
        return ("libggml.dylib", "libggml-base.dylib", "libggml-cpu.dylib")
    return (
        "libggml.so",
        "libggml.so.0",
        "libggml-base.so",
...
@dataclass(frozen=True)
class RuntimeLibraryDirectory:
    """An existing CUDA runtime directory and why it was selected."""

    path: Path
    source: str


def _package_root(module_name: str) -> Path | None:
    """Return an import package directory without importing the package itself."""
...
def discover_runtime_library_dirs() -> list[RuntimeLibraryDirectory]:
    """Find CUDA libraries in deterministic priority order.

    An explicit qwentts override wins, followed by NVIDIA's pip packages.
    Torch and a system CUDA installation are fallbacks. Only existing
    directories are returned and duplicate paths retain their first source.
    """

    candidates: list[tuple[Path, str]] = []

    override = os.environ.get("QWENTTS_CPP_CUDA_LIBRARY_PATH")
    if override:
        for value in override.split(os.pathsep):
            if value:
                candidates.append((Path(value), "QWENTTS_CPP_CUDA_LIBRARY_PATH"))

    platform_subdirs = ("bin", "lib", "lib64") if sys.platform == "win32" else ("lib", "lib64")
    for module_name, label in (
        ...
        if root is not None:
            for subdir in platform_subdirs:
                candidates.append((root / subdir, label))

    torch_root = _package_root("torch")
    if torch_root is not None:
        candidates.extend(
            (torch_root / subdir, "torch fallback")
            for subdir in (("lib",) if sys.platform != "win32" else ("lib", "bin"))
        )
    ...
    for env_name in ("CUDA_PATH", "CUDA_HOME"):
        ...
        if value:
            root = Path(value)
            for subdir in (("bin", "bin/x64", "lib/x64", "lib64") if sys.platform == "win32" else ("lib64", "lib")):
                candidates.append((root / subdir, f"{env_name} fallback"))

    search_env = "PATH" if sys.platform == "win32" else "LD_LIBRARY_PATH"
    for value in os.environ.get(search_env, "").split(os.pathsep):
        if value:
            candidates.append((Path(value), f"{search_env} fallback"))
    if sys.platform.startswith("linux"):
        candidates.extend(
            (Path(value), "system fallback")
            for value in ("/usr/local/cuda/lib64", "/usr/lib/x86_64-linux-gnu", "/usr/lib64")
        )

    result: list[RuntimeLibraryDirectory] = []
    seen: set[str] = set()
    for path, source in candidates:
        try:
            if not path.is_dir():
                continue
            has_cuda_library = any(
                any(path.glob(pattern))
                for patterns in _cuda_runtime_patterns(None)
                for pattern in patterns
            )
            if source != "QWENTTS_CPP_CUDA_LIBRARY_PATH" and not has_cuda_library:
                continue
            normalized = str(path.resolve()).casefold() if sys.platform == "win32" else str(path.resolve())
        except OSError:
            continue
        if normalized in seen:
            continue
        seen.add(normalized)
        result.append(RuntimeLibraryDirectory(path=path.resolve(), source=source))
    return result


def _cuda_runtime_patterns(cuda_major: int | None = 12) -> Sequence[Sequence[str]]:
    if sys.platform == "win32":
        major = str(cuda_major) if cuda_major is not None else "*"
        return (
    ...
    if sys.platform.startswith("linux"):
        major = str(cuda_major) if cuda_major is not None else "*"
        return (
            (f"libcudart.so.{major}", f"libcudart.so.{major}.*"),
            (f"libcublasLt.so.{major}", f"libcublasLt.so.{major}.*"),
            (f"libcublas.so.{major}", f"libcublas.so.{major}.*"),
        )
    return ()


def _detect_cuda_major(native_directory: Path) -> int:
    """Infer the CUDA ABI linked by the bundled GGML backend without loading it."""

    names = ("ggml-cuda.dll",) if sys.platform == "win32" else (
        "libggml-cuda.so",
        "libggml-cuda.so.0",
    )
    patterns = (
        re.compile(rb"cublas64_(\d+)\.dll", re.IGNORECASE),
        re.compile(rb"libcublas\.so\.(\d+)", re.IGNORECASE),
    )
    for name in names:
        candidate = native_directory / name
    ...
    # Released MVP wheels are CUDA 12; use that contract when a backend
    # does not expose its linked SONAME in a directly inspectable file.
    return 12


def _native_exception(message: str, *, status: int | None = None) -> QwenTTSError:
    detail = message or "The native qwentts.cpp operation failed without a diagnostic"
    lowered = detail.lower()
    if status == QwenStatus.OOM or any(
        token in lowered
        for token in ("out of memory", "allocation failed", "failed to allocate", "cudamalloc", "cuda_error_out_of_memory")
    ):
        return OutOfMemoryError(
            f"{detail} Reduce max_batch, use Q8_0/Q4_K_M, shorten codec_chunk_sec, or free GPU memory."
        )
    if "abi" in lowered and ("version" in lowered or "mismatch" in lowered):
        return ABIMismatchError(f"{detail} Install a wheel built for qwentts.cpp ABI {QT_ABI_VERSION}.")
    cuda_markers = (
        "cuda driver",
...
class QwenLibrary:
    ...
    def __init__(self, library_path: str | os.PathLike[str] | None = None):
        ...
        self.preloaded_library_sources: dict[Path, str] = {}
    ...
    def _add_dll_directory(self, path: Path) -> None:
        if sys.platform != "win32" or not hasattr(os, "add_dll_directory"):
            return
        try:
            # The returned handle controls the directory's lifetime. Keep it
            # for as long as this QwenLibrary instance can call native code.
            self._dll_dir_handles.append(os.add_dll_directory(str(path)))
        except OSError as exc:
            self._preload_errors.append(f"add_dll_directory({path}): {exc}")

    def _preload_cuda_runtime(self, mode: int) -> None:
        for patterns in _cuda_runtime_patterns(self.cuda_major):
            loaded = False
            for directory in self.runtime_library_dirs:
                candidates: list[Path] = []
                for pattern in patterns:
                    candidates.extend(sorted(directory.path.glob(pattern)))
                seen: set[Path] = set()
                for candidate in candidates:
                    try:
                    ...
                    seen.add(resolved)
                    try:
                        self._dependency_handles.append(ctypes.CDLL(str(candidate), mode=mode))
                    except OSError as exc:
                        self._preload_errors.append(f"preload {candidate}: {exc}")
                        continue
                    self.preloaded_libraries.append(candidate)
                    self.preloaded_library_sources[candidate] = directory.source
                    loaded = True
                    break
                if loaded:
                    break
    ...
    def _load_cdll(self, path: Path) -> ctypes.CDLL:
        ...
        self._add_dll_directory(path.parent)
        for directory in self.runtime_library_dirs:
        ...
        for dep_name in _dependency_names():
            ...
            if dep.is_file():
                try:
                    ...
                    self._preload_errors.append(f"preload {dep}: {exc}")
        try:
            return ctypes.CDLL(str(path), mode=mode)
        except OSError as exc:
            directories = "\n".join(
                f"  - {item.path} ({item.source})" for item in self.runtime_library_dirs
            ) or "  - none found"
            preload_detail = "\n".join(f"  - {item}" for item in self._preload_errors[-8:])
            if preload_detail:
                preload_detail = f"\nDependency preload errors:\n{preload_detail}"
            raise LibraryLoadError(
                f"Found qwentts.cpp at {path}, but Windows/Linux could not load it: {exc}\n"
    ...
    @staticmethod
    def _read_default_params(function, struct_type, label: str):
        # Default initializers write sizeof(native struct). A deliberately
        # oversized buffer lets us read and reject a future ABI safely instead
        # of letting an incompatible native library overrun a ctypes object.
        storage = ctypes.create_string_buffer(max(4096, ctypes.sizeof(struct_type)))
        pointer = ctypes.cast(storage, ctypes.POINTER(struct_type))
        function(pointer)
        native_abi = ctypes.cast(storage, ctypes.POINTER(ctypes.c_int)).contents.value
    ...
    def _verify_abi(self) -> int:
        params = self._read_default_params(self._lib.qt_init_default_params, QtInitParams, "initializer")
        return int(params.abi_version)

    def default_init_params(self) -> QtInitParams:
        return self._read_default_params(self._lib.qt_init_default_params, QtInitParams, "initializer")

    def default_tts_params(self) -> QtTTSParams:
        return self._read_default_params(self._lib.qt_tts_default_params, QtTTSParams, "TTS initializer")
...
class QwenTTS:
    ...
    def close(self) -> None:
        ...
        # lock. Shutdown therefore cannot free GPU state underneath an active
        # synthesis or voice-reference extraction.
        with self._lock:
            if self._ctx:
    ...
    def num_codebooks(self) -> int:
        if not self.library._has_qt_num_codebooks:
            raise ABIMismatchError(
                f"qt_num_codebooks is unavailable; cached RVQ references require qwentts.cpp ABI v{QT_ABI_VERSION}"
            )
        with self._lock:
    ...
    def speaker_names(self) -> list[str]:
        if not (self.library._has_qt_n_speakers and self.library._has_qt_speaker_name):
            raise ABIMismatchError(f"Speaker enumeration requires qwentts.cpp ABI v{QT_ABI_VERSION}")
        with self._lock:
            context = self._require_ctx()
            count = int(self.library._lib.qt_n_speakers(context))
            names: list[str] = []
            for i in range(count):
                value = self.library._lib.qt_speaker_name(context, i)
                if value:
    ...
    def extract_voice_ref(self, ref_audio_24k: np.ndarray) -> VoiceRef:
        ...
        if not (self.library._has_qt_extract_voice_ref and self.library._has_qt_voice_ref_free):
            raise ABIMismatchError(
                f"qt_extract_voice_ref is unavailable; voice reference extraction requires qwentts.cpp ABI v{QT_ABI_VERSION}"
            )
        ...
        try:
            if rc != QwenStatus.OK:
                raise _native_exception(
                    self.library.last_error() or f"qt_extract_voice_ref failed with status {rc}",
                    status=int(rc),
                )
    ...
    def synthesize(
        ...
        *,
        ...
        if rc != QwenStatus.OK:
            ...
            raise _native_exception(
                self.library.last_error() or f"qt_synthesize failed with status {rc}",
                status=int(rc),
            )
    ...
    def stream(
        ...
        *,
        ...
        def is_cancelled() -> bool:
            return internal_cancel_event.is_set() or (cancel_event is not None and cancel_event.is_set())
        ...
        def producer() -> None:
            ...
            try:
                ...
                if rc != QwenStatus.OK and not is_cancelled():
                    chunks.put(_native_exception(
                        self.library.last_error() or f"qt_synthesize failed with status {rc}",
                        status=int(rc),
                    ))
    ...
    def _make_tts_params(
        ...
        *,
        ...
        params = self.library.default_tts_params()

        if (ref_spk_emb is not None or ref_codes is not None) and params.abi_version < QT_ABI_VERSION:
            raise ABIMismatchError(f"Cached speaker/RVQ references require qwentts.cpp ABI v{QT_ABI_VERSION}")
